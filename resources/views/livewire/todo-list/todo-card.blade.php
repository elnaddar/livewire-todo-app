<div wire:key='{{ $todo->id }}'
    ...
    <div class="flex justify-between space-x-2">
        <div class="flex items-center">
            @if ($todo->id == $editTodoId)
                <input type="text" placeholder="Todo.."
                    class="bg-gray-100  text-gray-900 text-sm rounded block w-full p-2.5" wire:model='editNewName'>
                @error('editNewName')
                    <span class="block text-xs text-red-500">{{ $message }}</span>
                @enderror
            @else
                <input wire:click='toggle({{ $todo->id }})' class="mr-2" id="item-{{ $todo->id }}"
                    type="checkbox" {{ $todo->completed ? 'checked' : '' }}>
                <label for="item-{{ $todo->id }}"
                    class="text-lg text-gray-800 text-semibold">{{ $todo->name }}</label>
            @endif
        </div>
        ...
        <div class="flex items-center space-x-2">
            @if ($todo->id != $editTodoId)
                <button wire:click='toggleEdit({{ $todo->id }})'
